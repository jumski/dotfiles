...
function wt_capture
    ...
    _wt_show_help_if_requested $argv "Usage: wt capture [worktree-name] [--switch] [--force] [--claude-session[=ID]] [--no-claude-session]
...
Examples:
  ...
  wt capture myfeature                    # Worktree name = 'myfeature'
  wt capture --switch                     # Capture and switch, migrate most recent session
  ...
  wt capture myfeature --switch --yes     # Custom name, switch, auto-confirm
...
Note:
  To create a worktree for a different branch, use: wt new <branch-name>"
    ...
    if test "$claude_session_mode" != "skip"
        echo -e "\033[32m  ✓\033[0m Migrate Claude session to new worktree"
    else
    ...
    # ============================================================
    # PHASE 3: RESOLVE CLAUDE SESSION (interactive if needed)
    # ============================================================

    # Build Claude project directory paths
    ...
    # ============================================================
    # PHASE 4: EXECUTE (cd to repo_root, restore on any error)
    # ============================================================
